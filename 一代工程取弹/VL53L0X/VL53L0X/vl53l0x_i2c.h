...
////IO方向设置
//#define VL_SDA_IN()  {GPIOE->MODER&=~(3<<(2*2));GPIOE->MODER|=0<<2*2;}	//PB5输入模式
//#define VL_SDA_OUT() {GPIOE->MODER&=~(3<<(2*2));GPIOE->MODER|=1<<2*2;}    //PB5输出模式



////IO操作函数	 
//#define VL_IIC_SCL    PEout(3) 		//SCL
//#define VL_IIC_SDA    PEout(2) 		//SDA	 
//#define VL_READ_SDA   PEin(2) 		    //输入SDA 

//IO方向设置
#define VL_SDA_IN()  {GPIOE->MODER&=~(3<<(3*2));GPIOE->MODER|=0<<3*2;}	//PB5输入模式
#define VL_SDA_OUT() {GPIOE->MODER&=~(3<<(3*2));GPIOE->MODER|=1<<3*2;}    //PB5输出模式



//IO操作函数	 
#define VL_IIC_SCL    PEout(2) 		//SCL
#define VL_IIC_SDA    PEout(3) 		//SDA	 
#define VL_READ_SDA   PEin(3) 		    //输入SDA 
